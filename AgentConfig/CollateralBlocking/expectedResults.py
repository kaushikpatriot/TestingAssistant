...
class expectedResultLine(BaseModel):
    ...
    4. Final remainder becomes allocated/unallocated amounts
    """
    ...
    allocated: float = Field(
        description="""
        TRADING ALLOCATION: Amount allocated to provide trading limits and margins.

        Calculation Logic:
        - Applied after ALL obligations (MLN + OB categories) are satisfied
        - Converted to trading buying power/limits for the member
        - Directly impacts member's trading capacity
        - Primary purpose of collateral after meeting safety requirements

        Formula: Remaining collateral after (MLN + all OB categories)
        """
    )
    ...
    unallocated: float = Field(
        ...
        Formula: total_collateral_amount - (all blocked amounts + all allocations - all borrowed amounts)
...
class expectedResult(BaseModel):
  result: list[expectedResultLine]
...
class verificationResult(BaseModel):
  ...
  correction: str = Field(description="Description of what is wrong and what needs to be corrected")
